...
Trong lúc OpenClaw làm việc, bot vừa cập nhật một status message và heartbeat mỗi 60 giây, vừa gửi trực tiếp từng cập nhật assistant, thao tác tool, kết quả tool, trạng thái worker và phản hồi cuối của sub-agent vào kênh Discord. Phản hồi cuối của phiên chính vẫn đi qua response embed riêng để không gửi trùng. Embed hiển thị context chính xác của session theo số token đầy đủ, ưu tiên snapshot fresh của OpenClaw và fallback về usage provider gần nhất trong transcript; số estimate không được trình bày như usage chính xác. Nếu status của job đang chạy bị tin nhắn người dùng đẩy lên trên, bot tạo bản cập nhật ở cuối channel rồi xóa bản cũ; các thông báo do chính bot gửi không làm status bị tạo lại liên tục. Bot theo dõi transcript của phiên cha cùng mọi session con, kể cả assistant text có `stopReason: toolUse` và `stopReason: stop`. Chỉ dòng rõ ràng `MEDIA:<đường dẫn tuyệt đối>` mới được xem là thành phẩm; tham số của tool `image` và screenshot kiểm tra nội bộ không bao giờ tự gửi. Mỗi file hợp lệ được gửi ngay khi xuất hiện, không chờ đủ cả lô.

Trạng thái job, transcript offset, request fingerprint, durable task và delivery ledger được lưu nguyên tử tại `data/jobs.json`. Cursor tin nhắn Discord được lưu riêng theo từng kênh, vì vậy khi tiến trình bot khởi động lại, bot giữ nguyên session OpenClaw và quét bù các chat thường được gửi trong lúc offline; lệnh cũ không được tự phát lại. Bot cũng dò đúng user message trong transcript và gửi nốt phản hồi final chưa delivery trước khi cân nhắc chạy recovery prompt. Task còn chạy được reattach; task đã xong được quét nốt artifact chưa gửi; task bị mất chỉ được tự khôi phục một lần sau prompt xác minh UI. Nếu không chắc chắn, job kết thúc có blocker thay vì gửi lại prompt mù quáng.
