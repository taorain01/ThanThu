/**
 * ?votepvpsolo - Bình chọn lịch PvP Solo
 * Vote ngày + giờ cho 2 ngày PvP Solo trong tuần
 * 
 * Usage:
 * - ?votepvpsolo - Tạo bình chọn (24h)
 * - ?votepvpsolo end - Kết thúc sớm
 */
...
function createResultEmbed(guildId) {
    const embed = new EmbedBuilder()
        .setColor(0x2ECC71)
        .setTitle('🏆 KẾT QUẢ BÌNH CHỌN PVP SOLO')
        .setTimestamp();
...
function createPollEmbed(guildId, endTime, creator) {
    ...
    return new EmbedBuilder()
        .setColor(0x9B59B6)
        .setTitle('🏆 BÌNH CHỌN LỊCH PVP SOLO')
        .setDescription([
...
function createMenus() {
    ...
    // Buttons
    rows.push(new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('votepvp_result').setLabel('📊 Chi tiết').setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('votepvp_end').setLabel('🛑 Kết thúc').setStyle(ButtonStyle.Danger)
    ));
...
async function handleVote(interaction) {
    ...
    const key = interaction.customId.replace('votepvp_', '');
...
async function handleButton(interaction) {
    const guildId = interaction.guild.id;
    const action = interaction.customId.replace('votepvp_', '');
...
async function endPoll(client, guildId, channel) {
    ...
    try {
        ...
        const msg = await ch.messages.fetch(poll.messageId);
        await msg.edit({
            embeds: [new EmbedBuilder().setColor(0x95A5A6).setTitle('🏆 BÌNH CHỌN PVP SOLO ĐÃ KẾT THÚC').setTimestamp()],
            components: []
        });
    } catch (e) { }
...
async function execute(message, args) {
    ...
    if (activePolls.has(guildId)) {
        return message.reply('❌ Đã có bình chọn đang chạy! Dùng `?votepvpsolo end` để kết thúc.');
    }
...
module.exports = {
    name: 'votepvpsolo',
    aliases: ['votepvp'],
    description: 'Bình chọn lịch PvP Solo',
    execute,
    handleVote,
    handleButton,
    activePolls
