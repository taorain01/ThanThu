...
const _sessionNotifDedup = new Map(); // key: day -> { hash, sentAt }
...
async function checkMemberPresence(client) {
  ...
  try {
    ...
    console.error('[checkMemberPresence] L�i fetch members:', e.message);
  }
    ...
    try {
      ...
      if (channel) {
        ...
        const embed = new EmbedBuilder()
          .setColor(0xFF4444)
          .setTitle('�a�️ Phát hi�!n thành viên �ã rời Discord')
      ...
      console.error('[checkMemberPresence] L�i gửi thông báo:', e.message);
    }
  }
}
...
const statusList = [
  ...
  { name: 'Đang thư�xng trà �x Tuý Hoa Lâu', type: ActivityType.Watching },
  { name: 'Đang b�9p �x Cửu Lưu Môn', type: ActivityType.Playing },
 ...
 * Chạy khi bot kh�xi ��"ng �Ồ �ảm bảo setTimeout không b�9 mất sau restart
 */
        ...
        setTimeout(async () => {
          try {
            ...
            if (channel) {
              ...
              const embed = new EmbedBuilder()
                .setColor(0x2ECC71)
                .setTitle(`�S& BANG CHIẾN ${dayName.toUpperCase()} ĐÒ TỰ Đ��NG KẾT TH�aC!`)
                .setDescription(`⏰ Đã 23:00 - Bang Chiến **${dayName}** tự ��"ng kết thúc.`)
                .setTimestamp();
              await channel.send({ embeds: [embed] });
            }
          } catch (e) {
            console.error('[ready] L�i re-scheduled auto-end:', e.message);
          }
        }, msUntilCleanup);
...
module.exports = {
  ...
  async execute(client) {
    ...
    // �S& Đ�ng b�" lại position từ role Discord thật -> SQLite/Supabase
    ...
    // �S& Kh�xi tạo Supabase sync cho Web Bang Chiến
    ...
    if (supaOk) {
      ...
      if (guild) {
        ...
        try {
          ...
          console.error('[Supabase] L�i sync users:', userSyncErr.message);
        }
        ...
        supaSync.listenForWebChanges(guild.id, async (newData) => {
          ...
          if (newData._inserted) {
            try {
              ...
              if (!dayConfig) { console.log(`[Supabase] �a�️ Ngày không hợp l�!: ${day}`); return; }

              // KiỒm tra �ã có session cho ngày này chưa
              const existing = db.getActiveBangchienByDay(guild.id, day);
              if (existing) { console.log(`[Supabase] �a�️ Session ${day} �ã t�n tại trong SQLite, bỏ qua INSERT`); return; }

              // Lấy kênh BC �ã set bằng ?setbc
              const bcChannelId = db.getConfig(`bc_channel_${guild.id}`);
              if (!bcChannelId) {
                console.log(`[Supabase] �a�️ Chưa set kênh BC (?setbc). Không thỒ tạo session từ web.`);
                return;
              }
              ...
              // Nếu �ã có overview �  xóa cũ r�i gửi m�:i. Nếu chưa có �  tạo m�:i luôn.
              ...
              console.error('[Supabase] �R Xử lý web INSERT l�i:', err.message);
            }
            return;
          }
          ...
          if (newData._deleted) {
            try {
              ...
              if (newData._deleted_unknown && newData.remainingDays) {
                ...
                console.log(`[Supabase] �x️ Fallback DELETE: tìm ${sessionsToDelete.length} session cần xóa`);
              } else if (newData.day) {
                const found = sessions.find(s => s.day === newData.day);
                if (found) sessionsToDelete = [found];
                console.log(`[Supabase] �x️ Web �ã xóa BC session ${newData.day}`);
              }
              ...
              console.error('[Supabase] �R Xử lý web DELETE l�i:', err.message);
            }
            return;
          }
          ...
          try {
            ...
            console.error('[Supabase] �R Sync ngược l�i:', syncBackErr.message);
          }
        });
      }
    }

    // Kh�xi tạo notifications từ file
    thongbao.initializeNotifications(client);

    // Kh�xi tạo YenTiec Time Change Reminder
